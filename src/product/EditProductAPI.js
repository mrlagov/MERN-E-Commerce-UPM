...
import {photo, readAPI, update} from './api-productAPI.js'
...
class EditProductAPI extends Component {
  constructor({match}) {
    super()
    this.state = {
      name: '',
  ...
  componentDidMount = () => {
    /*photo({
      ...
      if(data.error){
        imgProfile="/api/products/defaultphoto"
      }
    })*/
    ...
    readAPI({
      ...
      if (data.error) {
        ...
        this.setState({_id: data._id, name: data.name, type:data.type, steps: data.steps, URL: data.URL,description: data.description, image: data.image, category: data.category, quantity:data.quantity, price: data.price});
      }
    })
  }
  clickSubmit = () => {
    ...
    return (<div>
      <Card className={classes.card}>
        <CardContent>
          ...
          <Avatar src={"http://localhost:3000/api/products/API/photo/"+this.state._id} className={classes.bigAvatar}/><br/>
...
EditProductAPI.propTypes = {
  classes: PropTypes.object.isRequired
}

export default withStyles(styles)(EditProductAPI)
