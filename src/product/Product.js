...
import {readURL, listRelatedURL} from './api-productURL.js'
import {readAPI, listRelatedAPI} from './api-productAPI.js'
...
class Product extends Component {
  ...
  loadProduct = (productId) => {
    readURL({productId: productId}).then((data) => {
      if (data.error) {
        readAPI({productId: productId}).then((data) => {
          if (data.error) {
            ...
            listRelatedAPI({
              ...
              if (data.error) {
                ...
                this.setState({imageUrl: `http://localhost:3000/api/products/API/photo/${this.state.product._id}`})
              }
            })
         }
        })
        ...
        listRelatedURL({
          ...
          if (data.error) {
            ...
            this.setState({suggestions: data})
            this.setState({imageUrl: `http://localhost:3000/api/products/URL/photo/${this.state.product._id}`})
          }
        })
     }
    })
            ...
            <Grid item xs={7} sm={7}>
              <Card className={classes.card}>
                <CardHeader
                  ...
                  subheader={this.state.product.quantity > 0 || this.state.product.type  === 'API' ? 'En Stock': 'Fuera de Stock'}
                  action={
                ...
                <div className={classes.flex}>
                  ...
                  <Typography component="p" type="subheading" className={classes.subheading}>
                    ...
                    <Link to={'/shops/'+this.state.product.shop._id} className={classes.link}>
                      <span>
                      <CardHeader
                          title={this.state.product.shop.name}
                        />
                      </span>
                    </Link>
                  </Typography>
