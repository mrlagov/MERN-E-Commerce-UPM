    ...
    return (<Card className={classes.card}>
      ...
      {this.state.cartItems.length>0 ? (<span>
          {this.state.cartItems.map((item, i) => {
            return <span key={i}><Card className={classes.cart}>
              <CardMedia
                ...
                image={'http://localhost:3000/api/products/'+item.product.type+'/photo/'+item.product._id}
              ...
              <div className={classes.details}>
                ...
                <div className={classes.subheading}>
                  ...
                  <TextField
                    ...
                    margin="normal"/>
                    <Button className={classes.removeButton} color="primary" onClick={this.removeItem(item.product)}>x Quitar</Button>
                </div>
              </div>
            </Card>
            <Divider/>
        ...
        <div className={classes.checkout}>
          <span className={classes.total}>Total: €{cart.getCartTotal()}</span>
          {!this.props.checkout && (auth.isAuthenticated()?
            <Button color="secondary" variant="raised" onClick={this.openCheckout}>Checkout</Button>
            :
            <Link to="/signin">
              <Button color="primary" variant="raised">Identificarse para checkout</Button>
            </Link>)}
      ...
      <Typography type="subheading" component="h3" color="primary">Ningun producto añadido al carrito.</Typography>
    }
    </Card>)
  }
}
