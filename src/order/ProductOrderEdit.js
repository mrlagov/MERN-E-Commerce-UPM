...
class ProductOrderEdit extends Component {
  ...
  loadStatusValues = () => {
    getStatusValues().then((data) => {
      if (data.error) {
        this.setState({error: "No pudo conseguirse el status"})
      } else {
  ...
  handleStatusChange = productIndex => event => {
    ...
    if(event.target.value === "Cancelado"){
        cancelProduct({
          ...
          .then((data) => {
              if (data.error) {
                this.setState({error: "Status no actualizado, intentalo de nuevo"})
              } else {
          ...
          return <span key={index}>
                  { item.shop === this.props.shopId &&
                    <ListItem button className={classes.nested}>
                      <ListItemText
                        primary={<div>
                                    <img className={classes.listImg} alt="" src={'http://localhost:3000/api/product/image/'+item.product._id}/>
